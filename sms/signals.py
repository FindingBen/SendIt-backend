...
from base.models import CustomUser, ContactList
...
def update_user_field(sender, instance, created, **kwargs):
    if created:
        ...
        contact_list = ContactList.objects.get(id=instance.contact_list.id)

        # Update the user's field with the desired value
        user.sms_count = user.sms_count - contact_list.contact_lenght
        user.save()
